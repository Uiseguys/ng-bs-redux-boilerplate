...
export class CwcList {
    ...
    interpolateText(vnode, obj) {
        if (vnode.vtext) {
            ...
            if (matches) {
                matches.map(matched => vnode.vtext = vnode.vtext.replace(matched, get(obj, matched.slice(2, -2), matched)));
            }
        }
        if (vnode.vattrs) {
            for (const key in vnode.vattrs) {
                if (vnode.vattrs.hasOwnProperty(key)) {
                    ...
                    if (matches)
                        matches.map(matched => vnode.vattrs[key] = vnode.vattrs[key].replace(matched, get(obj, matched.slice(1, -1), matched)));
                }
            }
        }
        return vnode;
